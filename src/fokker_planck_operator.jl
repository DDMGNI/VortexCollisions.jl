...
struct FokkerPlanckOperator{M, N, ℳ, 𝒩, RT, CT, HT, MT, WT} <:
       CollisionOperator{M, N, ℳ, 𝒩, RT, CT}
    grid::Grid2d{M, N, RT}
    ...
    function FokkerPlanckOperator{M, N, ℳ, 𝒩, RT, CT, HT, MT, WT}(
            grid::Grid2d{M, N, RT}, ft::FourierTransform{ℳ, 𝒩, RT, CT},
        ...
        𝔻 = [SharedArray{RT}((M, N)) for k in 1:2, l in 1:2]
...
function FokkerPlanckOperator(grid::Grid2d{M, N, RT}, ft::FourierTransform{ℳ, 𝒩, RT, CT};
        hfunc = hfunc_fokker_planck!, mfunc = mfunc_fokker_planck!,
        wfunc = wfunc_fokker_planck!) where {M, N, ℳ, 𝒩, RT, CT}
    op = FokkerPlanckOperator{
        M, N, ℳ, 𝒩, RT, CT, typeof(hfunc), typeof(mfunc), typeof(wfunc)}(
        grid, ft, hfunc, mfunc, wfunc)
    nworkers() > 1 ? initialize_workers(op, ft.ℳcut, ft.𝒩cut) : nothing
    return op
end

function initialize_workers(op::FokkerPlanckOperator{M, N, ℳ, 𝒩, RT, CT},
        ℳcut = 0, 𝒩cut = 0) where {M, N, ℳ, 𝒩, RT, CT}
...
@generated function collision_operator!(
        op::FokkerPlanckOperator{M, N, ℳ, 𝒩, RT, CT, HT, MT, WT},
        u::Matrix{RT}, divJ::Matrix{RT}) where {M, N, ℳ, 𝒩, RT, CT, HT, MT, WT}
    local ϕ::Matrix{RT}
    local h::Matrix{RT}
    local m::Matrix{RT}
...
function compute_convolution!(op::FokkerPlanckOperator{
        M, N, ℳ, 𝒩, RT, CT}) where {M, N, ℳ, 𝒩, RT, CT}
    if nworkers() > 1
        ...
        convolution_kernel_trapezoidal!(
            op.grid, 1, N, op.wfunc, op.Dh, op.Du, op.m, op.𝔽, op.𝔻)
    end
end

@generated function convolution_kernel_fourier!(
        gr::Grid2d{M, N}, ft::FourierTransform{ℳ, 𝒩, RT, CT},
        j1::Int, j2::Int, wfunc::Function,
        Dh::Vector{SharedArray{RT, 2}}, Dû::Vector{SharedArray{CT, 2}}, m̂::SharedArray{
            CT, 2},
...
@generated function convolution_kernel_trapezoidal!(
        gr::Grid2d{M, N}, j1::Int, j2::Int, wfunc::Function,
        Dh::Vector{SharedArray{RT, 2}}, Du::Vector{SharedArray{RT, 2}}, m::SharedArray{
            RT, 2},
        𝔽::Vector{SharedArray{RT, 2}}, 𝔻::Matrix{SharedArray{RT, 2}}) where {M, N, RT}
    local g::Vector{Matrix{RT}} = [zeros(RT, M, N), zeros(RT, M, N)]
    local w::Matrix{Matrix{RT}} = [zeros(RT, M, N) for k in 1:2, l in 1:2]
...
function mfunc_fokker_planck!(
        u::Matrix{RT}, m::Union{Array{RT, 2}, SharedArray{RT, 2}}, grid::Grid2d) where {RT}
    m .= 1
end

function hfunc_fokker_planck!(u::Matrix{RT}, ϕ::Matrix{RT},
        h::Union{Array{RT, 2}, SharedArray{RT, 2}}, grid::Grid2d) where {RT}
    h .= ϕ
end

@generated function wfunc_fokker_planck!(
        grid::Grid2d{M, N, RT}, i::Int, j::Int, g::Vector{Matrix{RT}},
        w::Matrix{Matrix{RT}}) where {M, N, RT}
